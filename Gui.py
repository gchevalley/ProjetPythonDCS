...
def manage_monitor(mon, tl_monitor):
    ...
    btn_del_idea_selected = Button(frame_monitor, text="Delete selected")
    ...
    for idea in mon.ideas:
        ...
        if tmp_ticker.is_valid:
            ...
            vec_data_ideas.append([tmp_idea_cb_value, tmp_ticker.symbol])
            
            tmp_label = Label(frame_monitor, text=tmp_ticker.last_price)
            tmp_label.grid(row=grid_row, column=1)
            
            tmp_label = Label(frame_monitor, text=tmp_ticker.daily_return)
            tmp_label.grid(row=grid_row, column=2)
            grid_row += 1
